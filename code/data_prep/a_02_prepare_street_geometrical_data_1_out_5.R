...
# read/transform lion streets (LION geographic base file of New York City streets)
  ...
  for (i in seq_along(1:nrow(streets_context))) { # for each street segment in the spatial context
    ...
    if(as.numeric(sf::st_length(streets_context[i,])) > 0) { # if street segment is longer than 0 m
      ...
      if(length(buffers) == 2){ # check if buffers are available
        ...
        # calculate street width and street mean building height only if there are buildings at both sides of the street segment (todo: streets that do not have buildings on each side are assigned width from lion?)
