# aim: street_sidewalk model from main analysis using open streets from transportation alternatives instead of NYC DOT
...
cns_trct_open_streets <- readRDS(paste0(generated.data.folder, "cns_trct_open_streets.rds"))
cns_trct_open_streets_df <- cns_trct_open_streets
sf::st_geometry(cns_trct_open_streets_df) <- NULL

# assign zero presence to open streets located in census tracts without active open streets according to TA survey 
cns_tracts_no_active_os <- cns_trct_open_streets_df[cns_trct_open_streets_df$op_st_active == 0, "GEOID"]
open_streets_prep$presence[which(open_streets_prep$GEOID %in% cns_tracts_no_active_os)] <- 0
open_streets_prep$perc_area[which(open_streets_prep$GEOID %in% cns_tracts_no_active_os)] <- 0
...
# add treated column to census tract (=1 for active open streets ever) in the open streets data
...
# set intervened as zero for days in which treated is not active
...
scaled <- dplyr::ungroup(op_st_noise) %>% 
  dplyr::select(mean_height, area_density_plan, pop_dens, perc.black, perc.hisp, perc.pov, day, perc_area) %>% 
...
data_scale <- cbind(op_st_noise[, c("GEOID", "date", "residential", "street_sidewalk", "commercial", "vehicle", "night_noise", "evening_noise", "afternoon_noise", "morning_noise", "treated", "intervened", "presence")],
                  scaled[,c("mean_height", "area_density_plan", "pop_dens", "perc.black", "perc.hisp", "perc.pov", "day", "perc_area")])
...
saveRDS(implem_all_cnstract_trad_street_sidewalk_interv, paste0(output.folder, "implem_all_cnstract_trad_street_sidewalk_interv_perc_area_s5.rds"))
