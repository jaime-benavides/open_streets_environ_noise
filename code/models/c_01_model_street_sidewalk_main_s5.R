...
## build dataset for model input
# get noise complaints data from study periods
noise_prep_diff <- noise_prep[c(dates_in_2019, dates_in_2021),]

# join noise complaints and open streets data
noise_prep_diff <- left_join0(noise_prep_diff, open_streets_prep[,c("GEOID", "date", "presence", "perc_area")],
                 by = c("GEOID", "date"))

# add ses variables and geometrical variables to noise and open streets dataset
...
# create continuous variable from day 1 to last day of the study periods
...
# run model ###--- vdo comment: discuss with Jaime if he unscales again for interpreation ---###
all_openstreets_all_cnstract_trad_street_sidewalk_interv <- gamm4::gamm4(street_sidewalk ~ s(perc_area, k=5, fx=TRUE) + intervened + perc.pov + perc.black 
...
saveRDS(all_openstreets_all_cnstract_trad_street_sidewalk_interv, paste0(output.folder, "all_openstreets_all_cnstract_trad_street_sidewalk_interv_perc_area_s5.rds"))